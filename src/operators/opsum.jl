...
A collection of [`OpTerm`](@ref)s on a lattice with physical dimensions `ds`. The terms
are validated against `ds` on construction / `push!` (positions in range, every operator
matching the local dimension), so an `OpSum` is self-consistent by construction; it is
the input of `MPOHamiltonian(::OpSum)`.
"""
struct OpSum{V<:AbstractVector{Int}}
	data::Vector{OpTerm}
	ds::V
end
